      ...
      {/*<div id="services" className="w-full flex flex-col py-14 gap-10">
        ...
        <div className="grid grid-cols-3 max-sm:grid-cols-1 max-lg:grid-cols-2 w-full gap-10">
          {content.map((item) => (
            <div
              key={item._id}
              className="h-60 shadow md:h-80 overflow-hidden p-3 hover:border-teamwork-primary-orange hover:border group transition-all duration-500 ease-in-out"
            >
              <Image
                ...
                className="block group-hover:hidden transition duration-500 ease-in-out"
              />
              <h3>{item.title}</h3>
              <p>{item.content}</p>
            </div>
          ...
          {content.map((item) => (
            ...
            >
              <Image
                ...
                height={400}
                alt={item.image?.alt ?? "Beskrivelse av bilde mangler"}
                className="group-hover:opacity-0 transition duration-500 ease-in-out"
              />
              <div className="absolute pr-3 top-auto mt-3 transition-all duration-500 ease-in-out group-hover:top-0">
